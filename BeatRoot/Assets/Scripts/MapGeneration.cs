...
public class MapGeneration : MonoBehaviour
{
  ...
  public float TunnelHeight = 4.0f;
  ...
  private float CreateNewSegment(int count)
  {
    for (int i = 0; i < count; ++i)
    {
      ...
      float random = Random.Range(
        -Randomness + Mathf.Max(Randomness - minYDiff, 0.0f),
        Randomness - Mathf.Max(Randomness - maxYDiff, 0.0f)
      );

      // top spline
      Vector3 prev = _CenterLastSplinePosTop;
      Vector3 pos = new Vector3(prev.x + TunnelHeight, prev.y + random, 0.0f);
      _TopSpline.AddNode(new SplineNode(pos, pos + (pos - prev).normalized * Roughness));
      _CenterLastSplinePosTop = pos;

      // bottom spline
      prev = _CenterLastSplinePosBottom;
      pos = new Vector3(prev.x + TunnelHeight, prev.y + random, 0.0f);
      _BottomSpline.AddNode(new SplineNode(pos, pos + (pos - prev).normalized * Roughness));
